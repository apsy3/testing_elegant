  ...
  return (
    <main className="bg-white">
      <div className="container py-12">
        <h1 className="h1 mb-6">Your cart</h1>
        {items.length===0 ? <p className="small">Your cart is empty.</p> : (
          <div className="grid md:grid-cols-3 gap-8">
            <div className="md:col-span-2 space-y-4">
              {items.map(i=> (
                <div key={i.slug} className="grid grid-cols-12 gap-4 items-center border-b border-[#E5E7EB] pb-4">
                  <img src={i.img} className="col-span-3 rounded-md border border-[#E5E7EB]" />
                  <div className="col-span-9 flex items-center justify-between">
                    <div>
                      <div className="font-semibold">{i.name}</div>
                      <div className="small">₹{i.price.toLocaleString('en-IN')}</div>
                    </div>
            ...
            <aside className="card p-4 h-fit">
              ...
              <p className="small mt-2">Taxes & shipping at checkout (demo).</p>
              <button className="btn-primary w-full mt-3">Checkout (demo)</button>
            </aside>
          </div>
        )}
      </div>
