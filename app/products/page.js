...
export default function Products(){
  ...
  const products = useMemo(()=>{
    return catalog.filter(p => 
      ...
      (p.name.toLowerCase().includes(q.toLowerCase()) || p.short.toLowerCase().includes(q.toLowerCase()))
    );
  }, [q, cat]);
  ...
  return (
    <main className="container py-10">
      <header className="mb-4">
        ...
        <p className="text-bone/90">Browse the preview range.</p>
      </header>
      ...
      {products.length===0 && <p className="mt-6">No results. Try clearing filters.</p>}
    </main>
  )
}
